...
public class RSI extends MarketIndicator {
     ...
     * @param _numDays The number of days over which the RSI should be
     * calculated. Should ideally be 14 days.
     */
    ...
    public double calculateRS() {
        ...
        while (count1 < gains.size() && count1 <= numDays) {
            if (((gains.size() - 1) - count1) >= 0 && ((gains.size() - 1) - count1) < gains.size());
            {
        ...
        while (count2 < losses.size() && count2 <= numDays) {
            if (((losses.size() - 1) - count2) >= 0 && ((losses.size() - 1) - count2) < losses.size());
            {
